...
from Classes.classes import CustomScale
...
class PlaybackBar :
    def __init__(self,master,theme):
        ...
        self.Endvar.set('0:00:00')

    def CreatePlaybackBar(self):
        ...
        img_slider = tk.PhotoImage(file="slider.png")
        img_trough = tk.PhotoImage(file="through.png")
     ...
     # create images used for the theme (instead of using the data option, it is 
        ...
        self.PlaybackbarLabel = tk.Frame(self.main,height=6,relief='raised',bg='#f4f4f4')
        self.PlaybackbarLabel.pack(side=BOTTOM,fill=X)

        self.SliderLabel = tk.Frame(self.PlaybackbarLabel,height=1,bg=wid_bg)
        self.SliderLabel.pack(side="top",pady=2)

        self.SliderLabel.rowconfigure(0,weight=1)
     
        self.SliderLabel.columnconfigure((1),weight=1)

        self.StartDuration = tk.Label(self.SliderLabel,bg=wid_bg,textvariable=self.Startvar,font=("Bahnschrift",10,"normal"))
        self.StartDuration.grid(column=0,row=0,ipadx=5,padx=2,sticky="w")

        self.ProgressSlider = CustomScale(self.SliderLabel,length=600,orient='horizontal',)
        self.ProgressSlider.grid(column=1,row=0,padx=5)

        self.EndDuration = tk.Label(self.SliderLabel,bg=wid_bg,textvariable=self.Endvar,font=("Bahnschrift",10,"normal"))
        self.EndDuration.grid(column=2,row=0,ipadx=5,padx=2,sticky="e")

        self.BottomLabel = tk.Frame(self.PlaybackbarLabel,height=3,bg=wid_bg)
        self.BottomLabel.pack(side="bottom",expand=0,pady=4)

        self.WidgetsLabel = tk.Frame(self.BottomLabel,height=3,bg=wid_bg)
        self.WidgetsLabel.pack(side="bottom")

        self.WidgetsLabel.rowconfigure(0,weight=1)
        for i in range(0,8):
            self.WidgetsLabel.columnconfigure(i,weight=1)

        self.PlayButton = tk.Button(self.WidgetsLabel,image=self.Icons.PlayIcon,relief='flat',bd=0,borderwidth=0,bg=wid_bg,cursor='hand2')
        self.PlayButton.grid(row=0,column=0,sticky='nsew',padx=(0,12))
        
        self.PauseButton = tk.Button(self.WidgetsLabel,image=self.Icons.PauseIcon,relief='flat',bd=0,borderwidth=0,bg=wid_bg,cursor='hand2')
        self.PauseButton.grid(row=0,column=0,sticky='nsew',padx=(10,12))
        self.PauseButton.grid_remove()

        self.ResumeButton = tk.Button(self.WidgetsLabel,image=self.Icons.ResumeIcon,relief='flat',bd=0,borderwidth=0,bg=wid_bg,cursor='hand2')
        self.ResumeButton.grid(row=0,column=0,sticky='nsew',padx=(10,12))
        self.ResumeButton.grid_remove()

        self.PreviousButton = tk.Button(self.WidgetsLabel,image=self.Icons.PreviousIcon,relief='flat',bd=0,borderwidth=0,bg=wid_bg,cursor='hand2')
        self.PreviousButton.grid(row=0,column=1,sticky='nsew',padx=2)

        self.StopButton = tk.Button(self.WidgetsLabel,image=self.Icons.StopIcon,relief='flat',bd=0,borderwidth=0,bg=wid_bg,cursor='hand2')
        self.StopButton.grid(row=0,column=2,sticky='nsew',padx=2)

        self.NextButton = tk.Button(self.WidgetsLabel,image=self.Icons.NextIcon,relief='flat',bd=0,borderwidth=0,bg=wid_bg,cursor='hand2')
        self.NextButton.grid(row=0,column=3,sticky='nsew',padx=2)

        self.TogglePlaylistButton = tk.Button(self.WidgetsLabel,image=self.Icons.TogglePlaylistIcon,relief='flat',bd=0,borderwidth=0,bg=wid_bg,cursor='hand2')
        self.TogglePlaylistButton.grid(row=0,column=4,sticky='nsew',padx=(12,425))

        self.VolVar = tk.DoubleVar()

        self.VolumeSlider = CustomScale(self.WidgetsLabel,variable=self.VolVar,cursor="hand2",to=100,length=100,orient='horizontal')#,style='bg.Horizontal.TScale'
        self.VolumeSlider.grid(row=0,column=8)
        self.VolVar.set(50)
        # self.pygame.mixer_music.set_volume(0.50)